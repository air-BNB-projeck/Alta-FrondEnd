...
const Home = () => {
  ...
  useEffect(() => {
    const fetchStays = async () => {
      try {
        const response = await axios.get("https://immersive-dashapi.my.id/stays");
  ...
  return (
    <div className="w-screen h-screen top-0 overflow-x-hidden z-0">
      ...
      <div className="flex flex-wrap justify-center mt-10 gap-20 mb-10 p-3">
        {stays.length > 0 ? stays.map((stay) => <Card key={stay.id} images={stay.images} name={stay.name} price={stay.price} rating={stay.rating} />) : <p>Tidak ada data homestay yang tersedia.</p>}
      </div>
    </div>
  );
};
